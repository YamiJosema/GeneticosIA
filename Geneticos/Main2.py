...
#Aquí pasamos de nuestro tipo de grafo a uno que podamos usar para imprimir y dibujar por pantalla:
def ConvertirParaDibujar(Grafito):
    GrafoParaDibujar = networkx.Graph()
    color_map=[]
    for i in  range(0,len(Grafito.nodes)):
        GrafoParaDibujar.add_node(i) #Cogemos los nodos de nuestro grafo y los añadimos al nuevo
        color_map.append(Colores[Grafito.nodes[i].color]) #Añadimos cada color de cada nodo al mapa de colores del grafo
    for i in range(0,len(Grafito.nodes)):
        for arista in R_EDGES:
            if i == arista.i:
                #Para cada nodo cogemos SUS aristas (sus aristas serán las que sean iguales en nodo_a) y las añadimos al grafo
