...
def calcularFitness(individual, n):
    """
...
def selection_and_reproduction(population, paises):
    ...
    """
    fitness = [ (calcularFitness(i,n), i) for i in population] #Calcula el fitness de cada individuo, y lo guarda en pares ordenados de la forma (5 , [1,2,1,1,4,1,8,9,4,1])
    ...
    pprint =[(calcularFitness(i,n), i) for i in selected]
...
pprint =[(calcularFitness(i,n), i) for i in population]
...
for i in range(100):
    ...
    pprint =[(calcularFitness(j,n), j) for j in population]
